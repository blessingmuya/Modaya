    ...
    <section id="pipeline" className="border-t border-line/70 py-20">
      <div className="container-page">
        ...
        <p className="mt-3 max-w-2xl text-[15px] text-muted">
          Exports are computed on the server and stored as objects. The browser is a client, not a
          renderer.
        </p>
        <ol className="mt-10 grid gap-3">
          {PIPELINE.map((node, i) => (
            <li key={node.label} className="card flex flex-col gap-1.5 p-5 sm:flex-row sm:items-baseline sm:gap-6">
              <span className="mono flex w-8 shrink-0 text-xs text-faint">
                {String(i + 1).padStart(2, '0')}
              </span>
              <span
                className={`w-40 shrink-0 text-[14.5px] font-medium ${
                  node.tone === 'mint' ? 'text-mint-300' : 'text-ink-soft'
                }`}
              >
                {node.label}
              </span>
              <span className="text-[14px] leading-relaxed text-muted">{node.body}</span>
            </li>
          ))}
        </ol>
      </div>
