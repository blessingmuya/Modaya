  ...
  async function analyze() {
    ...
    if (!res.ok || !data.jobId) {
      setState((prev) => ({
        ...prev,
        jobStatus: "failed",
        error: data.error ?? "Could not start analysis.",
      }));
      return;
    }
    ...
    <div className="card p-5">
      ...
      {busy && (
        <div className="mt-4 h-1.5 w-full overflow-hidden rounded-full bg-surface-3">
          <div
            className="h-full rounded-full bg-mint-400 transition-[width]"
      ...
      {!profile && !busy && !state.error && (
        <p className="mt-4 rounded-lg bg-base-900 p-3.5 text-[12.5px] leading-relaxed text-muted ring-1 ring-line">
          Analyzing decodes the file server-side: frame-to-frame differences
          give cut points, the decoded audio gives loudness and onsets, sampled
          frames give colour statistics. All of it is deterministic and free.
          The written description needs an API key and is labelled separately.
        </p>
      )}
      ...
      {profile && (
        <div className="mt-5 grid gap-4">
          ...
          {variant === "full" && (
            <ModelBlock
              profile={profile}
              visionConfigured={state.visionConfigured}
            />
          )}
        </div>
      )}
    ...
    <section className="rounded-lg bg-base-900 p-4 ring-1 ring-line">
      ...
      {stale && (
        <p className="mt-3 rounded-md bg-warn-400/10 px-3 py-2 text-[11.5px] text-warn-400 ring-1 ring-warn-400/25">
          ...
          {missingFields.length > 0
            ...
            : ""}
          . Missing values are shown as — rather than guessed. Re-analyze to
          fill them in.
        </p>
      )}
      ...
      <dl className="mono mt-4 grid gap-x-6 gap-y-2 text-[12px] sm:grid-cols-2">
        ...
        <Row
          ...
          value={
            num(m.cutResolutionSec, 3, "s", "±") +
            (m.sampledFps ? ` (sampled at ${m.sampledFps}fps)` : "")
          }
        />
        <Row
          label="shot length (median)"
          value={num(m.shots?.medianSec, 2, "s")}
        />
        <Row
        ...
        <Row
          ...
          value={
            m.durationSec
              ? ((cutTimes.length / m.durationSec) * 60).toFixed(1)
              : "—"
          }
        />
        ...
        <Row
          label="active level"
          value={num(audio?.activeMeanRmsDb, 1, "dBFS")}
        />
        <Row label="peak level" value={num(audio?.peakRmsDb, 1, "dBFS")} />
        <Row
          label="mean level (incl. silence)"
          value={num(audio?.meanRmsDb, 1, "dBFS")}
        />
        <Row
          label="active / silent windows"
          value={
            audio
              ? `${pct(audio.activeRatio)} / ${pct(audio.silenceRatio)}`
              : "—"
          }
        />
        <Row
          ...
          value={
            audio
              ? `${audio.onsetsSec.length} (${num(audio.onsetsPerMinute, 1, "/min")})`
              : "—"
          }
        />
        <Row
        ...
        <Row
          ...
          value={
            grade
              ? `${num(grade.meanR, 1)} / ${num(grade.meanG, 1)} / ${num(grade.meanB, 1)}`
              : "—"
          }
        />
        <Row
          ...
          value={
            grade
              ? `${num(grade.meanLuma, 1)} / σ ${num(grade.contrast, 1)}`
              : "—"
          }
        />
  ...
  return (
    <section className="rounded-lg bg-base-900 p-4 ring-1 ring-line">
      <header className="flex flex-wrap items-center gap-2.5">
        ...
        <span className="text-[11.5px] text-faint">
          {model
            ? `${model.provider} · ${model.model} · ${model.frameCount} frames`
            : "not available"}
        </span>
      </header>
      ...
      {model ? (
        <div className="mt-4 grid gap-3.5 text-[12.5px] leading-relaxed">
          ...
          <Field label="Pacing (in words)">
            {text(model.pacingDescription)}
          </Field>
          ...
          <div className="rounded-md bg-surface-2 p-3 text-[11.5px] text-muted ring-1 ring-line">
...
function Field({
  ...
  return (
    <div>
      <span className="block text-[11.5px] tracking-wide text-faint uppercase">
        {label}
      </span>
