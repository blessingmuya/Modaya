...
type SourceInfo = Pick<
  MediaAsset,
  "id" | "filename" | "durationSec" | "width" | "height" | "fps"
>;
...
export function TimelineEditor({ projectId, sources, initialSpec }: Props) {
  ...
  const activeSource =
    sources.find((s) => s.id === activeAssetId) ?? sources[0];
  ...
  async function persist(nextClips: Clip[], note: string) {
    ...
    const res = await fetch(`/api/projects/${projectId}/timeline`, {
      ...
      body: JSON.stringify({
        spec: { version: 1, clips: nextClips },
        source: "manual",
        note,
      }),
    });
  ...
  function loadSource() {
    ...
    void persist(
      initialTimeline(activeSource.id, sourceDuration),
      `load ${activeSource.filename}`,
    );
  }

  function cutSelection() {
    const next = removeOutputRanges(clips, [
      { startSec: selection.start, endSec: selection.end },
    ]);
    if (next.length === 0) {
      setMessage(
        "That range covers the whole timeline — nothing would be left.",
      );
      return;
    }
    void persist(
      next,
      `cut ${selection.start.toFixed(2)}–${selection.end.toFixed(2)}s`,
    );
  }

  function keepSelection() {
    const next = trimTo(clips, {
    ...
    void persist(
      next,
      `trim to ${selection.start.toFixed(2)}–${selection.end.toFixed(2)}s`,
    );
  }
  ...
  async function startExport() {
    ...
    if (!res.ok || !data.jobId) {
      setRender({
        ...EMPTY_EXPORT,
        status: "failed",
        error: data.error ?? "Could not start the render.",
      });
      return;
    }
        ...
        <div className="hairline mt-5 grid gap-3 pt-5">
          ...
          {sources.length === 0 ? (
            <p className="text-[13px] text-faint">
              Upload footage above — it will be probed by the worker, then
              appear here.
            </p>
          ) : (
                  ...
                  <label className="flex min-w-0 cursor-pointer items-center gap-2.5">
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="mono block text-[11px] text-faint">
                        {source.durationSec
                          ? `${source.durationSec.toFixed(2)}s`
                          : "probing…"}
                        {source.width
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[12.5px] text-muted">
              Rendered server-side with ffmpeg at source resolution — not a
              browser capture.
            </p>
          </div>
          <button
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 text-[12.5px]">
              ...
              >
                ...
                {render.status === "running" &&
                  `Encoding on the server… ${render.progress}%`}
        ...
        {placed.map(({ clip, index, outStart, outEnd }) => (
          <button
            ...
            className={`group relative min-w-[36px] border-r border-base-950/70 px-1 text-left transition-colors ${
              index === activeIndex
                ? "bg-mint-400/30"
                : "bg-mint-400/12 hover:bg-mint-400/20"
            }`}
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        <label className="flex items-center gap-3">
          ...
          <input
            ...
            onChange={(e) =>
              onChange({
                start: Math.min(Number(e.target.value), value.end - step),
                end: value.end,
              })
            }
