...
export async function MediaList({
  ...
  const rows = await Promise.all(
    assets.map(async (asset) => ({
      asset,
      streamUrl: await presignGet(ctx, { key: asset.bucketKey, expiresIn: 60 * 60 * 6 }),
      ...
      {rows.map(({ asset, streamUrl, downloadUrl }) => (
        ...
        >
          <div className="min-w-0">
            <span className="block truncate text-[13.5px] text-ink">{asset.filename}</span>
            <span className="mono mt-0.5 block text-[11px] text-faint">
              {formatBytes(asset.sizeBytes)}
              {asset.width && asset.height ? ` · ${asset.width}×${asset.height}` : ''}
              {asset.durationSec ? ` · ${Number(asset.durationSec).toFixed(2)}s` : ''}
              {asset.fps ? ` · ${Number(asset.fps).toFixed(2)}fps` : ''} · {asset.status}
            </span>
          </div>
          <div className="flex shrink-0 items-center gap-2">
            {asset.kind === 'video' && (
              <a className="btn btn-ghost btn-sm" href={streamUrl} target="_blank" rel="noreferrer">
                Play
              </a>
            )}
