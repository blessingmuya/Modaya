  ...
  async function upload(file: File) {
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        xhr.onerror = () => reject(new Error('Upload failed — storage is unreachable.'));
        xhr.send(file);
      });

      setPhase('confirming');
      const confirm = await fetch(`/api/media/${signed.assetId}/complete`, { method: 'POST' });
      if (!confirm.ok) {
  ...
  return (
    <div className="card p-5">
      <div className="flex flex-wrap items-center gap-3">
          ...
          {(['source', 'reference'] as Role[]).map((r) => (
            <button
              ...
              className={`rounded-full px-3.5 py-1.5 text-[12.5px] transition-colors ${
                role === r ? 'bg-mint-400 font-medium text-base-950' : 'text-muted hover:text-ink'
              }`}
            >
        ...
        >
          {busy ? phaseLabel(phase) : `Choose ${role === 'source' ? 'footage' : 'a reference'}`}
        </button>
        ...
        <div className="mt-4">
          <div className="h-1.5 w-full overflow-hidden rounded-full bg-surface-3">
            <div
              ...
              style={{ width: `${phase === 'uploading' ? progress : phase === 'signing' ? 3 : 99}%` }}
            />
          </div>
        </div>
      )}
